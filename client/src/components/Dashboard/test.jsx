...
class Test extends Component {
  ...
  onFormSubmit(e) {
    ...
    let formD = {
      ...
      description: document.forms.namedItem('testCreate')['description'][
        'value'
      ],
      status: document.forms.namedItem('testCreate')['status']['value'],
      show_status: document.forms.namedItem('testCreate')['show_status'][
        'value'
      ],
    };
  ...
  getOneProject(e) {
    e.preventDefault();
    const urlGetOne =
      '/project/' + document.forms.namedItem('testOne')['id']['value'];
    axios
      .get(urlGetOne)
      .then((res) => alert(JSON.stringify(res.data['project'])));
  }

  deleteOneProject(e) {
    e.preventDefault();
    const urlDelete =
      '/project/' + document.forms.namedItem('deleteOne')['id']['value'];
  ...
  getAllProject(e) {
    ...
    axios
      .get('/project/')
      .then((res) => alert(JSON.stringify(res.data['projects'])));
  }
  ...
  onFormUpdate(e) {
    ...
    let formD = {
      ...
      show_status: document.forms.namedItem('testUpdate')['show_status'][
        'value'
      ],
    };
    if (document.forms.namedItem('testUpdate')['name']['value'].trim() !== '') {
      ...
      formD['name'] = document.forms
        .namedItem('testUpdate')
        ['name']['value'].trim();
    }
    if (
      document.forms.namedItem('testUpdate')['description']['value'].trim() !==
      ''
    ) {
    ...
    const reqUrl =
      '/project/update/' +
      document.forms.namedItem('testUpdate')['id']['value'];
    const result = axios.post(reqUrl, formD);
  }

  onUserUpdate(e) {
    e.preventDefault();
    const names = document.forms
      .namedItem('userUpdate')
      ['names']['value'].split(',');
    const updateUrl =
      'project/add_people/' +
      document.forms.namedItem('userUpdate')['id']['value'];
  ...
  onUserDelete(e) {
    e.preventDefault();
    const names = document.forms
      .namedItem('userDelete')
      ['names']['value'].split(',');
    const deleteUrl =
      'project/remove_people/' +
      document.forms.namedItem('userDelete')['id']['value'];
  ...
  onProcessCreate(e) {
    e.preventDefault();
    let formD = {
      processNum: document.forms.namedItem('processCreate')['processNum'][
        'value'
      ],
      description: document.forms.namedItem('processCreate')['description'][
        'value'
      ],
    ...
    const createUrl =
      'project/process/' +
      document.forms.namedItem('processCreate')['id']['value'];
  ...
  onProcessDelete(e) {
    ...
    const deleteUrl =
      ...
      document.forms.namedItem('processDelete')['id']['value'];
    axios.post(deleteUrl, {
      processNum: document.forms.namedItem('processDelete')['processNum'][
        'value'
      ],
    });
  }
  ...
  onProcessUpdate(e) {
    let formD = {
      processNum: document.forms.namedItem('processUpdate')['processNum'][
        'value'
      ],
    ...
    const updateUrl =
      ...
      document.forms.namedItem('processUpdate')['id']['value'];
  ...
  onCreateNode(e) {
    ...
    let formD = {
      processNum: parseInt(
        document.forms.namedItem('createNode')['processNum']['value']
      ),
      name: document.forms.namedItem('createNode')['name']['value'],
      description: document.forms.namedItem('createNode')['description'][
        'value'
      ],
    };
    let url =
      '/project/process/node/' +
      document.forms.namedItem('createNode')['id']['value'];
  ...
  onUpdateNode(e) {
    ...
    let formD = {
      processNum: parseInt(
        document.forms.namedItem('updateNode')['processNum']['value']
      ),
      nodeIndex: parseInt(
        document.forms.namedItem('updateNode')['processNum']['value']
      ),
    ...
    if (document.forms.namedItem('updateNode')['description']['value'] != '') {
      formD['description'] = document.forms.namedItem('updateNode')[
        'description'
      ]['value'];
    }
    let url =
      '/project/process/node/update/' +
      document.forms.namedItem('updateNode')['id']['value'];
    axios.post(url, formD);
  }
  onDeleteNode(e) {
    e.preventDefault();
    let formD = {
      processNum: parseInt(
        document.forms.namedItem('deleteNode')['processNum']['value']
      ),
      nodeIndex: parseInt(
        document.forms.namedItem('deleteNode')['nodeIndex']['value']
      ),
    };
    let url =
      '/project/process/node/remove/' +
      document.forms.namedItem('deleteNode')['id']['value'];
    axios.post(url, formD);
  }
  onFinishNode(e) {
    e.preventDefault();
    let formD = {
      processNum: parseInt(
        document.forms.namedItem('finishNode')['processNum']['value']
      ),
      nodeIndex: parseInt(
        document.forms.namedItem('finishNode')['nodeIndex']['value']
      ),
    };
    let url =
      '/project/process/node/finish/' +
      document.forms.namedItem('finishNode')['id']['value'];
    axios.post(url, formD);
  }
  conditionalSearch(e) {
    e.preventDefault();
    ...
    if (document.forms.namedItem('conditionalSearch')['byName'].checked) {
      formD.name = document.forms.namedItem('conditionalSearch')['name'][
        'value'
      ];
    }
    if (document.forms.namedItem('conditionalSearch')['byStatus'].checked) {
      formD.status = document.forms.namedItem('conditionalSearch')['status'][
        'value'
      ];
    }
    if (document.forms.namedItem('conditionalSearch')['sort'].checked) {
      formD.sortBy = document.forms.namedItem('conditionalSearch')[
        'sortMethod'
      ]['value'];
    }
  ...
  likeOtherProject(e) {
    e.preventDefault();
    let url =
      '/project/like/' + document.forms.namedItem('like')['id']['value'];
    axios.post(url);
  }
  ...
  createTimeLine(e) {
    ...
    const description = document.forms.namedItem('createTimeLine')[
      'description'
    ]['value'];
    const date = document.forms
      .namedItem('createTimeLine')
      ['date']['value'].split('-');
    const year = date[0];
    const month = date[1];
    const day = date[2];
    const time = document.forms
      .namedItem('createTimeLine')
      ['time']['value'].split(':');
    const hrs = time[0];
    const mins = time[1];
  ...
  render() {
    return (
      <div>
        ...
        <button onClick={this.getAllProject}>click me</button>
        <br />
        GetOneProject
        ...
        <form onSubmit={this.onUserUpdate} name="userUpdate">
          ...
          <input
            type="text"
            name="names"
            placeholder="username: luc_a,luc_b,luc_c..."
          />
          <input type="submit" value="test" />
        </form>
        DeleteUser
        ...
        <form onSubmit={this.onProcessCreate} name="processCreate">
          ...
          <input
            ...
            step="1"
            name="processNum"
            placeholder="processNum"
          />
          <input
            type="text"
            name="description"
            name="description"
            placeholder="processDescription"
          />
          <select name="status">
