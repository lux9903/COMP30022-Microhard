...
describe('Projects', () => {
  ...
  it('Add a contributor', async () => {
    ...
    expect(re.body).to.be.an.an('object');
    expect(re.body)
      .to.have.property('contributors')
      .that.include.members(['a', 'b', 'c', 'admin']);
  });
  ...
  it('Anoymous like one project', async () => {
    let re = await request(server).post('/api/project/like/anoymous/' + id1);
...
*/
  it('Create Timeline', async () => {
    ...
    assert.equalDate(
      new Date(re.body.timeline[0].time),
      new Date(1981, 11, 12, 0, 0, 0, 0)
    );
    assert.equalDate(
      new Date(re.body.timeline[1].time),
